...
describe('PGDBPlayer', function() {
    ...
    before(function(done) {
        ...
        pgdbPlayer.created()
            .then(function(data) { assert.equal(data, pgdbPlayer); })
    ...
    it('should take and return a username', function (done) {
        ...
        pgdbPlayerName.created()
            .then(function(data) {
                assert.equal(pgdbPlayerName.username(), "unknown");
                return pgdbPlayerName.setUsername("Jerry");
            })
            .then(function(data) { assert.equal(pgdbPlayerName.username(), "Jerry"); })
            .fail(function(err)  { assert.fail(err); })
            .done(function()     { done(); });
    });
    ...
    it('should reject finding unknown username', function (done) {
        pgdbPlayer.find("unknown")
            ...
            .fail(function() { /* otherwise rejection gets thrown */ })
            .done(function() { done(); });
    });
    ...
    it('should have a player named "computer"', function(done) {
        pgdbPlayer.find('computer')
            .fail(function(err) { assert.fail("cannot find computer " + err); })
            .done(function() { done(); });
    });
